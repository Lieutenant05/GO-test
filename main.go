...
func check_type(i string) (string, int) {
	...
	if slices.Contains(ar_num, i) {
		...
		fmt.Println("Числа должны быть арабскими целыми: от 1 до 10; или римскими: от I до X")
		panic("Числа должны быть арабскими целыми: от 1 до 10; или римскими: от I до X")
	}
}
...
func oper_check(op string) string {
	...
	if op == "+" {
		...
		fmt.Println("Введён неверный оператор")
...
func main() {
	...
	if len(strp_st) == 1 { //При вводе без пробелов работает не корректно. В примерах работы программы использовался ввод с пробелами, поэтому багом не считаю.
		...
		if ir == 0 && ir2 == 0 {
			...
			if itogo_ar < 1 {
				fmt.Println("В римской системе нет отрицательных чисел и ноля")
				panic("В римской системе нет отрицательных чисел и ноля")
			} else {
